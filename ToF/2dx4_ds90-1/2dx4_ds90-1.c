...
/* Those basic I2C read functions can be used to check your own I2C functions */
  ...
  status = VL53L1X_StartRanging(dev);   /* This function has to be called to enable the ranging */
	...
	uint8_t round=0;
	while(1){
	GPIO_PORTL_DATA_R=0b00100000;		//external LED on
	if((GPIO_PORTE_DATA_R&0b00000001)==0b00000000){ //button pressed
		GPIO_PORTL_DATA_R=0b00000000;	//external LED off
		for(uint32_t i=0;i<=222;i++){	//one cycle
			if (i!=0&&i%3==0){					//check for 3 steps
				status = VL53L1X_GetDistance(dev, &Distance);
				FlashLED4(1);
				
				deg=(float)i/222.00*2*3.141592653;
				x=Distance*sin(deg);
				y=Distance*cos(deg);
